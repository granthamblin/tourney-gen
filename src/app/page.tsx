...
export default function Home() {
  ...
  const [numFlips, setNumFlips] = useState<number>(10);

  const shuffleArray = (array: string[]): string[] => {
    return array.sort(() => 0.5 - Math.random());
  };

  const getRandomItems = (category: string[], itemCount: number): string[] => {
    const shuffled = shuffleArray(category);
    return shuffled.slice(0, itemCount);
  };

  const validateInput = (): boolean => {
    if (isNaN(numFlips) || numFlips <= 0 || numFlips > 20) {
      alert("Please enter a valid number of flips (between 1 and 20).");
      return false;
    }
  ...
  const generateRound = () => {
    ...
    const selectedCapstips = getRandomItems(capstips, Math.ceil(numFlips * 0.25));
    const selectedMultis = getRandomItems(multis, Math.ceil(numFlips * 0.25));
    const selectedStacks = getRandomItems(stacks, Math.ceil(numFlips * 0.25));
    const selectedMoms = getRandomItems(moms, Math.ceil(numFlips * 0.25));
    ...
    while (allSelectedItems.length < numFlips) {
      ...
      allSelectedItems.push(...shuffleArray(remainingItems).slice(0, numFlips - allSelectedItems.length));
    }

    setSelectedItems(shuffleArray(allSelectedItems).slice(0, numFlips));
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-24 gap-2">
      <div className="font-bold font-mono">MARKER FLIP TOURNEY</div>
      <div className="font-bold font-mono">SEASON #2</div>

      <div className="flex flex-row gap-5 mt-5">
        <div className="flex flex-col items-center">
          <label htmlFor="your-name" className="block text-gray-700 font-bold font-mono mb-2 text-center">NAME #1</label>
          <input
            id="your-name"
            type="text"
        ...
        <div className="flex flex-col items-center">
          <label htmlFor="opponent-name" className="block text-gray-700 font-mono font-bold mb-2 text-center">NAME #2</label>
          <input
            id="opponent-name"
            type="text"
            className="text-black text-center font-mono w-full p-2 border border-gray-300 rounded"
     ...
     <label htmlFor="num-flips" className="block text-gray-700 font-bold font-mono mb-2 text-center">NUMBER OF FLIPS</label>
      <input
      id="num-flips"
      type="number"
      className="text-black text-center font-mono w-15 p-2 border border-gray-300 rounded"
      value={numFlips}
